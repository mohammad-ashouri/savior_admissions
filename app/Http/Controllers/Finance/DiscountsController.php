...
class DiscountsController extends Controller
{
    ...
    public function index()
    {
        ...
        $this->logActivity(json_encode(['activity' => 'Getting Discounts']), request()->ip(), request()->userAgent(), session('id'));
    ...
    public function edit($id)
    {
        ...
        $this->logActivity(json_encode(['activity' => 'Getting Academic Year Discounts Information For Edit', 'discount_id' => $id]), request()->ip(), request()->userAgent(), session('id'));
    ...
    public function store(Request $request)
    {
        ...
        if (empty($discounts)) {
            $this->logActivity(json_encode(['activity' => 'Saving Discounts Failed', 'errors' => 'Access Denied', 'discount_id' => $request->discount_id]), request()->ip(), request()->userAgent(), session('id'));

            abort(403);
        }
        if (count($request->name) != count($request->percentage)) {
            $this->logActivity(json_encode(['activity' => 'Saving Discounts Failed', 'errors' => 'Data Count Is Not Equal', 'discount_id' => $request->discount_id]), request()->ip(), request()->userAgent(), session('id'));
        ...
        $this->logActivity(json_encode(['activity' => 'Discounts Saved', 'discount_id' => $request->discount_id]), request()->ip(), request()->userAgent(), session('id'));
    ...
    public function getDiscountPercentage(Request $request)
    {
        ...
        $this->logActivity(json_encode(['activity' => 'Getting Discount Percentage', 'discount_id' => $request->discount_id]), request()->ip(), request()->userAgent(), session('id'));
